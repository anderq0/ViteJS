...
function getAbbreviation(str: string) {
        return str.replaceAll('-',' ').split(' ').map(i => i[0].toUpperCase()).join('')
}
...
function stringCalcualtor(str: string) {
    ...
    console.log(stringCalculate)
    for (let el = 0; el < stringCalculate.length; el++) {
        if (stringCalculate[el] == '+') {
            let result = (+stringCalculate[el - 1] + +stringCalculate[el + 1])
            return stringCalculate.splice(el-1,3,result)
...
console.log(stringCalcualtor('3678 + 2'))
